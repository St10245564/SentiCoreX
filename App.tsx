...
const ResultCard: React.FC<{ result: SentimentAnalysisResult; onDeeperAnalysis: () => void; index: number }> = ({ result, onDeeperAnalysis, index }) => {
    ...
    return (
        <div className="border-2 border-gray-light dark:border-gray-dark rounded-2xl p-6 mb-4 transition-all hover:shadow-2xl hover:scale-[1.02] bg-white dark:bg-gray-darker/80 animate-slide-up" style={{ animationDelay: `${index * 100}ms` }}>
...
const ResultsSection = React.forwardRef<HTMLDivElement, { results: SentimentAnalysisResult[], onDeeperAnalysis: (result: SentimentAnalysisResult) => void, onExport: (format: 'pdf'|'csv'|'json')=>void }>(({ results, onDeeperAnalysis, onExport }, ref) => {
    return (
        <div className="bg-white dark:bg-gray-darker/80 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-gray-light dark:border-gray-dark animate-slide-up" style={{ animationDelay: '200ms' }}>
            ...
            <div ref={ref} className="max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                {!results.length ? (
...
const App: React.FC = () => {
    ...
    const handleAnalyze = useCallback(async (input: string | string[]) => {
        ...
        if (analysisCount + analysesToRun > MAX_ANALYSES) {
            ...
            setApiStatus({ status: 'error', message: `Analysis limit reached. You can perform ${remaining > 0 ? remaining : 'no'} more ${remaining === 1 ? 'analysis' : 'analyses'}.` });
            return;
        }
    ...
    const handleCompare = useCallback(async (textA: string, textB: string) => {
        if (analysisCount >= MAX_ANALYSES) {
             setApiStatus({ status: 'error', message: `Analysis limit of ${MAX_ANALYSES} reached. You cannot perform a comparison.` });
             return;
        }
        ...
        try {
            ...
            setComparisonModalState({ isOpen: true, result: null, isLoading: false, error: error.message || 'Failed to compare texts.' });
        } finally {
            setIsLoading(false);
        }
    }, [analysisCount]);
    ...
    const handleDeeperAnalysis = useCallback(async (result: SentimentAnalysisResult) => {
        ...
        try {
            ...
            setModalState(s => ({ ...s, isLoading: false, error: 'Failed to perform deeper analysis.' }));
        }
    }, []);
     ...
     const handleExport = useCallback((format: 'csv' | 'json' | 'pdf') => {
        ...
        if (format === 'json') {
            ...
            const input = resultsContainerRef.current;
            if (input) {
                const originalStyle = input.style.maxHeight;
                input.style.maxHeight = 'none'; // Temporarily un-restrict height for full capture
                // FIX: Call html2canvas from the window object.
                window.html2canvas(input, { scale: 2 }).then(canvas => {
                    input.style.maxHeight = originalStyle; // Restore original style
                    ...
                    input.style.maxHeight = originalStyle;
                });
            }
        }
    }, [results]);
